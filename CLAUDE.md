...
npm run docs       # audit the documentation, then build it
npm run docs:audit # coverage audit only (§38 of DOCUMENTATION_SPEC.md)
npm run docs:build # docs/*.md -> docs/site/ (static, gitignored)
npm run docs:serve # build, then serve the doc site on :4180
```

> **Documentation : suivre `DOCUMENTATION_SPEC.md`. Une tâche n'est « done » que si la doc
> est à jour.**
...
- The prompt therefore **embeds the whole catalogue** (220 lines, ~12 KB, generated from
...
All 152 exercises have a demo. `demoFor(id)` still returns `null` for an unknown id and both
...
`DOCUMENTATION_SPEC.md` is the source of truth for the documentation, and the rule it exists
to enforce is that **a feature is not done while its documentation is stale**. The user-facing
docs live in `docs/*.md` (French, one page per screen / feature / problem) and are read in
**two places, from the same files**: a static site built into `docs/site/` (generated,
gitignored, never hand-edited) and the app's own *Documentation* screen.

**There is no second copy, deliberately.** `data/docs.js` globs `../../docs/**/*.md` and the
app renders those files directly, so a page cannot be current on the site and stale in the
app. The same discipline applies to the renderer: `src/lib/markdown.js` holds the Markdown
subset and its renderer, React-free precisely so `scripts/gen-docs.mjs` can import it, and
`data/docSections.js` holds the chapter list both consume. A second implementation of either
would drift the first time a page used something it did not support.

- **`npm run docs:audit` reads the code, not the docs' own claims.** That is the whole point:
  nothing else in the build fails when a screen ships undocumented. It checks that every
  screen in `App.jsx` is claimed by a guide page's `couvre:` front matter, that every key of
  `defaultProfile` (plus `voiceOn`, `theme`, `openrouter.*`) appears in
  `docs/reference/settings.md`, that every browser capability actually used has a permission
  entry, that every `throw new Error` / `setError` message appears in
  `docs/reference/errors.md`, that every `https://` host reachable from `src/` is named in
  `docs/data/`, that the catalogue counts quoted in the docs still match the catalogues, and
  that every internal link and anchor resolves. It exits non-zero, so it can gate a commit.
- **Adding a screen means adding its `couvre:` line**, exactly as adding a screen already
  means adding a `help.js` entry. The audit fails both ways — a page claiming a screen that
  no longer exists is also an error.
- `scripts/docs-lib.mjs` carries a small Markdown subset renderer rather than a dependency:
  §4 of the spec says not to introduce a technology just for the docs, and the subset is
  exactly what `docs/` uses. It supports `{#ancre}` on a heading, which is what keeps the
  doc anchors equal to the in-app FAQ ids.
- **`docs/screenshots/` belongs to the README** and is generated by `npm run shots`; the doc
  site *copies* it to `docs/site/assets/screenshots/` rather than moving it.
- **The documentation is a lazily imported chunk (~236 KB), and must stay one.** Naming it in
  `manualChunks` turns it into a *manual* chunk, which Vite then `modulepreload`s from
  `index.html` — that downloads the whole reference on every cold start for a screen most
  sessions never open. The dynamic `import()` in `lib/docs.js` already produces the chunk
  *and* names it `docs`; there is nothing to add. `docs:audit` fails if an entry comes back.
  Unlike the zxing decoder it **is** precached: the Dépannage chapter is what someone needs
  when something is broken, which is often when they have no network.
- **A doc link inside the app is a state change, not a navigation.** `render()` takes an
  `href` mapper — the site turns `x.md` into `x.html`, the app leaves it alone and
  `Documentation.jsx` intercepts the click, resolving through `resolveDocPath` (the same
  function `docs:audit` checks links with, so a link that passes the audit cannot dead-end in
  the app).
- `Documentation.jsx` is the one place in `src/` using `dangerouslySetInnerHTML`. The
  Markdown is authored in this repository and bundled at build time; no user input and no
  remote content ever reaches it.
- The doc site is **not** wired into the app's deploy. It is a standalone folder, on purpose:
  serving it under the app's own origin would be swallowed by the service worker's
  `navigateFallback`, and adding a documentation URL to `src/` would fail
  `check-catalogue`'s host grep unless the privacy policy named that host too.
